...
const AGENTS_T=[
 ...
 {ag:"Policy Simulation Agent",ag_zh:"政策模拟引擎",ic:"🛡️",lyr:"L3",io:{in:"Gap + 6-measure toolkit",out:"Recommend Combo #4 · 97%"},lines:[
   {k:"think",t:"Systematic simulation. Objective: maximize Seg-A gap closure with feasible, demand-matched supply; constraint is the closure plan's own delivery budget envelope.",t_zh:"做系统性政策模拟。目标:以可落地、需求匹配的供给最大化 A 段缺口闭合;约束是闭合方案自身的交付预算包。"},
 ...
 {ag:"Governance Gate · Human-in-the-Loop",ag_zh:"治理门 · 人工复核",ic:"!",lyr:"L4",gate:true,io:{in:"Draft recommendation",out:"Awaiting human approval"},lines:[
   ...
   {k:"think",t:"The gap-closure plan (supply additions + actions) needs sign-off before dispatch",t_zh:"供需缺口闭合方案(新增供给 + 执行动作)在分发前需签核"},
...
const AGENT_ACTIONS=[
 ...
 {key:"fin",  name:"Financial Sustainability (derived)",name_zh:"财务可持续性(衍生)",acts:["link_gap_closure","note_revenue_loop","note_cost_transfer","flag_dss_handoff"]},
];
...
// AI Agent-theater parameter panel — drives scenario/coverage/target inputs for the gap-closure run.
// (Financial sustainability is a DERIVED by-product of closing the gap, not a primary score.)
const FISCAL_BUDGET=7.9, CONTRACT_TARGET=510000, CONTRACT_COST=141444;
const FISCAL_DEFAULTS={burn:67,eff:0.82,hbr:36,leak:5,mape:7,contracts:127952,scn:"base",hor:70};
...
function FiscalContinuityPanel({compact=false,lv:lvP,setLv:setLvP,active=false}){
  const {lang,cov,setCov}=useStore();
  ...
  return (<div className={"fiscal-card"+(compact?" compact":"")+(active?" synced":"")}>
    ...
    <div className="fiscal-controls">
      <div className="fcfg">
        ...
        <div className="seg2">{[["ministry",L("Ministry","部委")],["private",L("Private","私有")],["total",L("Total","全市场")]].map(c=><button key={c[0]} className={cov===c[0]?"on":""} onClick={()=>setCov(c[0])}>{c[1]}</button>)}</div>
        <small className="fnote">{L("Ministry 95% / Private 80% / Total","部委 95% / 私有 80% / 全市场")}</small>
      </div>
      <div className="fcfg">
      ...
      <label className="fslider">
        <span><b>{L("Target homeownership","目标住房拥有率")}</b><em style={{color:lv.hor>71?"var(--amber)":"var(--green-dark)"}}>{lv.hor}%</em></span>
        <input type="range" min={64} max={74} step={.5} value={lv.hor} onChange={e=>set("hor",Number(e.target.value))}/>
        <small className="fnote">{L("now 66.2% · Vision-2030 target 70% · >71% is aggressive","当前 66.2% · Vision 2030 目标 70% · >71% 偏激进")}</small>
      </label>
      <label className="fslider">
...
function ChatAnalysis(){
    ...
    return (<div className="fade">
      ...
      <div className="ai-brief-grid">
        ...
        <div className="ai-brief-main card pad">
          ...
          <h2>{L("SAMA +50bps shock → Seg-A supply-demand gap → gap-closure plan","SAMA +50bps 冲击 → A 段供需缺口 → 缺口闭合方案")}</h2>
  ...
  const backendStage=(curKey==="orch"||curKey==="dq")&&!ai.done; // system logic — no analyst input (final output excluded)
  const askAgain=()=>{ setNote(""); start(); };
  ...
  return (<div className="fade">
    ...
    <div className="theater theater-fiscal">
      ...
      {ai.on&&!ai.gate&&!(AGENTS_T[ai.ag]&&AGENTS_T[ai.ag].gate)&&<div className={"agent-dock"+(backendStage?" backend":"")}>
        <span className="dock-ic">{backendStage?"🔒":(ai.done||ai.awaiting?"✦":"")}</span>
        <input className="dock-input" value={backendStage?"":note} disabled={backendStage||(!ai.awaiting&&!ai.done)}
          placeholder={backendStage?L("Backend stage — analyst input not allowed (orchestration & data-quality are system logic)","后台环节 —— 不接受分析师补充(编排 / 数据质量为系统逻辑)"):(ai.done?L("Ask a follow-up / new question…","继续提问 / 智能问数…"):(ai.awaiting?L("Add a suggestion to re-run this agent…","补充建议,重新生成本 Agent…"):L("Agent is analyzing…","Agent 分析中…")))}
          onChange={e=>setNote(e.target.value)} onKeyDown={e=>{if(e.key!=="Enter"||backendStage||!note.trim())return; if(ai.done)askAgain(); else if(ai.awaiting)regenerate();}}/>
        {ai.done
          ? <button className="btn sm" onClick={askAgain} disabled={!note.trim()} title={L("Run a new analysis with this question","以该问题重新发起分析")}>Enter</button>
