    ...
    <div className="theater theater-fiscal">
      ...
      <div className="tstream" ref={streamRef}>
        ...
        {ai.gate&&<div className="gate-card">
          ...
          <div className={"gate-fiscal "+(gateApprove?"pass":"fail")}>{L("Scenario","情景")}: <b>{flv.scn==="opt"?L("Optimistic","乐观"):flv.scn==="pess"?L("Pessimistic","悲观"):L("Base","基准")}</b> · {L("target","目标")} <b>{flv.hor}%</b> · {gateApprove?L("fundable — approval enabled","可承受 — 可批准"):(flv.scn==="pess"?L("pessimistic stress — must escalate","悲观承压 — 须上报"):L("target too aggressive (>71%) — must escalate","目标过激进(>71%)— 须上报"))} <span className="muted">({L("set on the left panel →","在左侧面板设置 →")})</span></div>
